 ...
 * ask the assistant, chat about the rules…). Pass `onClick` for an action or
 * `href` for navigation. Pass `className="sm:hidden"` for a mobile-only FAB.
 */
export function Fab({
  onClick,
  href,
  ...
  return (
    <button onClick={onClick} aria-label={label} title={label} className={classes}>
      <Icon className="h-6 w-6" />
    </button>
  );
}
